...
class EditTickets extends StatefulWidget {
  ...
  EditTickets({required this.GetData});
...
class _EditTicketsState extends State<EditTickets> {
  ...
  // Future<List<dynamic>?>? allticketdata;

  // Future<List<dynamic>?>? getTickets() async {
  //   Uri uri = Uri.parse(UrlResourece.GETTICKETS);
  //   var responce = await http.get(uri);
  //   if (responce.statusCode == 200) {
  //     var body = jsonDecode(responce.body);
  //     print("body = ${body}");
  //     setState(() {
  //       ticketTypes = body['data'];
  ...
  geteventdata()async{
    ...
    setState(() {
      ...
      // tickets = widget.GetData!["event_ticket_id"].toString();
      description.text = widget.GetData!["ticket_description"].toString();
      price.text = widget.GetData!["ticket_price"].toString();
      total.text = widget.GetData!["total_ticket"].toString();
      // Capitalize the first letter of the ticket name to match dropdown items
      String ticketName = widget.GetData!["ticket_name"].toString();
      tickets = ticketName[0].toUpperCase() + ticketName.substring(1).toLowerCase();

      print("ticketType = $tickets");
    });
  }
  List<String> ticketTypes = ['Gold', 'Silver', 'Bronze'];
                                        ...
                                        children: [
                                          ...
                                          eventsData.isNotEmpty
                                            ...
                                            value: events != null &&
                                                ...
                                                ? events
                                                : null,
                                            items: eventsData.map((val) {
                                              return DropdownMenuItem<String>(
                                                value: val['event_id'].toString(),
                                                child: Text(
                                                    val['event_title'].toString()),
                                              );
                                            }).toList(),
                                            onChanged: (val) {
                                              setState(() {
                                    ...
                                    InkWell(
                                      onTap: () async {
                                        if (formkey.currentState!.validate()) {
                                          ...
                                          var params = {
                                            "event_ticket_id": ticketid.text.trim(), // <-- This line is essential for update
                                          ...
                                          print("Params: $params");
                                          Uri url = Uri.parse(UrlResourece.UPADTETICKETS);
